...
# lokalnych gałęzi, które zostały zmergowane do master lub są starsze niż 90 dni.
...
PROTECTED_BRANCHES=("master" "main" "develop" "DEV")
...
trap 'echo "Sprzątanie katalogu tymczasowego..."; rm -rf "$TEMP_DIR"' EXIT

REPO_URL="https://bitbucket.org/${BITBUCKET_USER}/${REPO_NAME}.git"

echo "Klonowanie repozytorium z $REPO_URL..."
# Klonujemy repozytorium do katalogu tymczasowego
...
# Używamy `tr -d ' *'` do usunięcia gwiazdki i białych znaków
...
# Używamy `git for-each-ref` do iteracji po zdalnych gałęziach i sprawdzenia daty ostatniego commita
while IFS= read -r line; do
    commit_date_iso=$(echo "$line" | awk '{print $1}')
...
done < <(git for-each-ref --sort=committerdate refs/remotes/origin/ --format='%(committerdate:iso) %(refname:short)' | sed 's/origin\///')
